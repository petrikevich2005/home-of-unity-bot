...
def randomize_prayers(message: telebot.types.Message) -> None:
    if (
        not is_banned(message.from_user.id) and
...
def send_message_for_all_users(message: telebot.types.Message) -> None:
    if (
        ...
        bot.send_message(
            message.from_user.id, replies["message"]["success"].format(count=count)
        )
...
def send_message_for_all_mod_plus(message: telebot.types.Message) -> None:
    if (
        not is_banned(message.from_user.id) and 
        get_user_role(message.from_user.id) >= Role.ADMIN.value
    ):
            ...
            if user_utils.check_username(resours[0]):
                if get_user_role(message.from_user.id) >= resours[1]:
                    if get_user_role(message.from_user.id) >= get_user_role(
                        ...
                        if (
                            get_user_role(get_id_using_username(resours[0]))
                            < Role.DEV.value
                            or get_user_role(get_id_using_username(resours[0]))
                            == Role.DEV.value
    ...
    if role >= Role.ADMIN.value and not ban_status:
        ...
        if user_utils.check_username(username):
            ...
            if role >= other_role:
                ...
                logger.info(
                    f"user {message.from_user.id} banned user "
                    f"{other_user_id}"
                )
    ...
    if role >= Role.ADMIN.value and not ban_status:
        ...
        if user_utils.check_username(username):
            ...
            if role >= other_role:
                ...
                logger.info(
                    f"user {message.from_user.id} unbanned user "
                    f"{other_user_id}"
                )
                if str(message.from_user.id) != other_user_id:
                    bot.send_message(
