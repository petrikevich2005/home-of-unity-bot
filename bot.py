					...
					if user[0] == str(message.from_user.id):
						...
						if user[1] == 1:
							...
							send_message_to_specific_category_users(
								...
								Roles.ADMIN.value, message.from_user.id,
							)
...
def change_event_state(message):
	...
	try:
		...
		if not is_baned(message.from_user.id, "event"):
			...
			if (
				user_utils.check_username(message.from_user.username)
				and message.from_user.username is not None and False
			):
				with sqlite3.connect("data.db") as cursor:
					users = cursor.execute(
...
def who_i_am(message):
	...
	try:
		...
		if complete:
			...
			logger.warning(f"not found role of user {message.from_user.id}! (@{message.from_user.username})")
			bot.send_message(
				message.from_user.id,
				"""
				Ошибка!
			...
			if not is_baned(message.from_user.id, "text"):
				...
				# SEND MESSAGE TO ALL USERS
					...
					count = send_message_to_specific_category_users(
						text, Roles.DEFAULT.value, message.from_user.id,
					)
				...
				# SEND MESSAGE TO MODERATORS AND ADMINS
					...
					text = message.text[1:]
					count = send_message_to_specific_category_users(text, Roles.MOD.value, message.from_user.id)
						...
						if not resours:
							...
							if user_utils.check_username(resours[0]):
								if get_user_category(message.from_user.id) >= resours[1]:
									if (
										get_user_category(message.from_user.id) >=
										...
										if (
											get_user_category(get_id_using_username(resours[0])) < Roles.DEV.value or
											get_user_category(get_id_using_username(resours[0])) == Roles.DEV.value and
											str(message.from_user.id) == str(get_id_using_username(resours[0]))
										):
											try:
												bot.send_message(
										...
										send_message_to_specific_category_users(
											...
											f"@{resours[0]} на {resours[1]}", Roles.DEV.value, message.from_user.id,
										)
									else:
										logger.debug(f"user {message.from_user.id} role is lower than user @{resours[0]}")
										bot.send_message(
											message.from_user.id, "Вы не можете изменить уровень доступа этого пользователя," +
						...
						if message.text[4] == " ":
							...
							if user_utils.check_username(username):
								if (
									...
									with sqlite3.connect("data.db") as cursor:
										cursor.execute("UPDATE users SET ban = ?, state = ? WHERE username = ?", (1, 0, username))
									...
									if get_user_category(get_id_using_username(username)) != Roles.DEV.value:
										try:
											bot.send_message(get_id_using_username(username), "Вы заблокированы.")
										except Exception:
				...
				# SEND HELP LIST
					...
					if role >= Roles.ADMIN.value:
						bot.send_message(
							message.from_user.id, f"{config.MODERATOR_HELP_LIST}\n{config.ADMIN_HELP_LIST}",
						)
